...
class ModelSaleCustomer extends Model {
	...
	public function editCustomer($customer_id, $data) {
        ...
        if($data['password'])
            $this->db->query("UPDATE " . DB_PREFIX . "customer SET email = '" . $this->db->escape($data['email']) . "', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', name = '" . $this->db->escape($data['name']) . "', sex = '" . (int)$data['sex'] . "', zone = '" . $this->db->escape($data['zone']) . "', place = '" . $this->db->escape($data['place']) . "', telephone = '" . (($data['telephone'])?(int)($data['telephone']):'') . "', qq = '" . (($data['qq'])?(int)($data['qq']):'') . "', wechat = '" . $this->db->escape($data['wechat']) . "', status = '" . (($data['status'])?((int)($data['status'])):'1') . "' WHERE customer_id = '" . (int)$customer_id . "'");
        else
            $this->db->query("UPDATE " . DB_PREFIX . "customer SET email = '" . $this->db->escape($data['email']) . "', name = '" . $this->db->escape($data['name']) . "', sex = '" . (int)$data['sex'] . "', zone = '" . $this->db->escape($data['zone']) . "', place = '" . $this->db->escape($data['place']) . "', telephone = '" . (($data['telephone'])?(int)($data['telephone']):'') . "', qq = '" . (($data['qq'])?(int)($data['qq']):'') . "', wechat = '" . $this->db->escape($data['wechat']) . "', status = '" . (($data['status'])?((int)($data['status'])):'1') . "' WHERE customer_id = '" . (int)$customer_id . "'");
