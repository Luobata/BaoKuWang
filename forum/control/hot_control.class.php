...
class hot_control extends common_control{
	...
	public function on_baike(){
		...
		if(sizeof($list)!=0){
			...
			//var_dump($list);
			//从thread表中获取内容
			$lists=$this->thread->index_fetch(array('fid'=>array('='=>$list[0]['fid']),'typeid1 '=>$list[0]['typeid']),array('dateline'=>-1),0,4);
			if(sizeof($lists)!=0){
				echo json_encode(array_values($lists));
			}else{
				$lists=$this->thread->index_fetch(array('fid'=>array('='=>$list[0]['fid']),'typeid2 '=>$list[0]['typeid']),array('dateline'=>-1),0,4);
				if(sizeof($lists)!=0){
					echo json_encode(array_values($lists));
				}else{
					$lists=$this->thread->index_fetch(array('fid'=>array('='=>$list[0]['fid']),'typeid3 '=>$list[0]['typeid']),array('dateline'=>-1),0,4);
					if(sizeof($lists!=0)){
						echo json_encode(array_values($lists));
					}
					else{
						$lists=$this->thread->index_fetch(array('fid'=>array('='=>$list[0]['fid']),'typeid4 '=>$list[0]['typeid']),array('dateline'=>-1),0,4);
						echo json_encode(array_values($lists));
					}
				}
			}
