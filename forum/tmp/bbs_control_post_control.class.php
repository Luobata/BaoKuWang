...
class post_control extends common_control {
		...
		if(!$this->form_submit()) {
			...
			if(!array_filter($error)) {
				...
				if($quote && $quote['uid'] != $this->_user['uid']) {
					...
					$pmmessage = "【{$this->_user['username']}】引用了您的帖子：<a href=\"http://localhost/forum/?thread-index-fid-$fid-tid-$tid-page-$page.htm\" target=\"_blank\">【{$pmsubject}】</a>。";
					...
					$pmmessage = "【{$this->_user['username']}】回复了您的主题：<a href=\"http://localhost/forum/?thread-index-fid-$fid-tid-$tid.htm\" target=\"_blank\">【{$pmsubject}】</a>。";
	...
	public function on_delete() {
		...
		if($isfirst) {
			...
			$this->location("http://localhost/forum/?forum-index-fid-$fid.htm");
			...
			$this->location("http://localhost/forum/?thread-index-fid-$fid-tid-$tid-page-$post[page].htm");
		}
	}
