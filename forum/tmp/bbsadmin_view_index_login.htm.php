...
<div class="width">
	<div class="div">
		...
		<div class="body">
			<form action="http://localhost/forum/admin/?index-login.htm" method="post" id="login_form">
				<input type="hidden" name="FORM_HASH" value="<?php echo FORM_HASH;?>" />
				<input type="hidden" name="referer" value="<?php echo isset($referer) ? $referer : '';?>" />
				<input type="hidden" name="clienttime" id="clienttime" value="0" />
				<dl>
					<dt><label for="login_email">用户名：</label></dt>
					<dd><?php echo isset($_user['username']) ? $_user['username'] : '';?></dd>
					
					<dt><label for="login_password">密　码：</label></dt>
					<dd><input type="password" name="password" id="password" size="24" value="" title="请输入密码" tabindex="11" style="width: 250px;" /></dd>
					
					<dt><label for="safe_admin_auth">安全选项：</label></dt>
					<dd><input type="checkbox" name="safe_admin_auth" id="safe_admin_auth" value="1" tabindex="13" /> 访问前台自动清空后台Cookie</dd>
					
					<dt></dt>
					<dd>
						<a type="submit" class="button bigblue" id="submit" value="提交" tabindex="12" href="javascript:void(0)" role="button"><span>提交</span></a>
...
$('#password').focus();
$('#clienttime').val(Math.round(new Date().getTime()/1000));
</script>
